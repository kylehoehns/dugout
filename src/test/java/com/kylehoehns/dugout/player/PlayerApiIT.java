...
class PlayerApiIT {
    ...
    void should_return_all_players_when_listing() throws Exception {
        ...
        // when
        var response = mockMvc.perform(get("/api/players")).andReturn().getResponse();
    ...
    void should_return_single_player_when_id_exists() throws Exception {
        ...
        // when
        var response = mockMvc.perform(get("/api/players/{id}", 1L)).andReturn().getResponse();
    ...
    void should_return_404_when_player_is_missing() throws Exception {
        ...
        // when
        var status =
                mockMvc.perform(get("/api/players/{id}", unknownId))
                        .andReturn()
                        .getResponse()
                        .getStatus();
    ...
    void should_return_matching_player_when_filtering_by_position_uppercase() throws Exception {
        ...
        // when
        var response =
                mockMvc.perform(get("/api/players").param("position", "SS"))
                        .andReturn()
                        .getResponse();
        var players = objectMapper.readValue(response.getContentAsString(), Player[].class);
    ...
    @Test
    @DisplayName(
            "returns only the matching player when filtering by position (lowercase, case-insensitive)")
    ...
    void should_return_empty_list_when_no_players_match_position() throws Exception {
        ...
        // when
        var response =
                mockMvc.perform(get("/api/players").param("position", "ZZ"))
                        .andReturn()
                        .getResponse();
        var players = objectMapper.readValue(response.getContentAsString(), Player[].class);
    ...
    void should_return_all_players_when_position_param_is_empty_string() throws Exception {
        ...
        // when
        var response =
                mockMvc.perform(get("/api/players").param("position", ""))
                        .andReturn()
                        .getResponse();
        var players = objectMapper.readValue(response.getContentAsString(), Player[].class);
